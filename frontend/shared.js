  ...
  return `
    <div style="margin-top:16px">
      ...
      ${step(2,'Embedding + Retrieval',retColor,[
        ...
        ['Branch signal',   !t.retrieval_similarity?'—':t.retrieval_similarity>=.95?'→ Cache':'→ LLM-RAG'],
      ],false)}
      ...
      ${step(5,'LLM Decision',verColor,[
        ['Decision score',   fmtNum(t.verifier_score,3)],
        ['Confidence score', t.confidence_score!=null?Math.round(t.confidence_score*100)+'%':'—'],
        ['Grounding signal', (t.verifier_score||0)>=.7?'✓ Strong':'Review if risk requires'],
...
function buildResolutionPlan(t) {
  ...
  const evidence = t.semantic_cache_hit
    ...
    : t.route_path === 'generative_rag'
      ? 'Resolution generated by the LLM from scored RAG evidence and retrieved historical context.'
...
function buildMatchedEvidence(t) {
  ...
  if (t.semantic_cache_hit) {
    ...
    label = '◉ Scored RAG evidence used by LLM'; labelColor = 'var(--accent)';
  } else if (!t.retrieval_similarity || t.retrieval_similarity < .70) {
    label = '⚠ Weak RAG evidence attached for LLM review'; labelColor = 'var(--warn)';
  } else {
